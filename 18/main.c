...
static gboolean on_idle(gpointer data) {
	...
	if(ball.dy < 0) {
		...
		if(
			ball.pos[0] >= paddle.pos[0]-paddle.width && 
			ball.pos[0] <= paddle.pos[0]+paddle.width &&
			ball.pos[1] - ball.radius >= paddle.pos[1]-paddle.height &&
			ball.pos[1] - ball.radius <= paddle.pos[1]
		  ) {
	...
	bl = ball.pos[0] - ball.radius;
	br = ball.pos[0] + ball.radius;
	bt = ball.pos[1] + ball.radius;
	bb = ball.pos[1] - ball.radius;
	...
	for(i = 0; i < 30; i++) {
			...
			if(
				((bl >= bricks.pos[i][0] && bl <= bricks.pos[i][0] + bricks.width) || 
				 (br >= bricks.pos[i][0] && br <= bricks.pos[i][0] + bricks.width)) &&
				((bt >= bricks.pos[i][1] - bricks.height && bt <= bricks.pos[i][1]) ||
				 (bb >= bricks.pos[i][1] - bricks.height && bb <= bricks.pos[i][1]))
			   ) {
				ball.dy = -ball.dy;
				bricks.active[i] = 0;
			}
