...
static void on_realize(GtkGLArea *area) {
	...
	for(i = 0; i < 25; i++) {
		...
		bricks.pos[i][0] = bricks.x_padding + (bricks.x_padding * col * 2);
		bricks.pos[i][0] += bricks.width + (bricks.width * 2 * col);

		bricks.pos[i][1] = bricks.y_padding + (bricks.y_padding * row);
		bricks.pos[i][1] += bricks.height + (bricks.height * 2 * row);
